        ...
        for bicolored_graph in bicolored_graphs.iter() {
            ...
            for &(row, col, num) in &graph.nodes() {
                ...
                for &(rowb, colb, numb) in graph.neighbors((row, col, num)).unwrap() {
                    visualizer_updates.push(VisualizerUpdate::CreateChain(row, col, num, rowb, colb, numb, Colors::CHAIN_COLOR));
                }
            ...
            // Rule 1 Twice in a cell
            {
                ...
                if let Some(color) = conflicting_color {
                    visualizer_updates.push(VisualizerUpdate::SetTitle("3d Medusa: Twice in a Cell".to_string()));
            ...
            // Rule 2 Twice in a Unit
            {
                ...
                if let Some(color) = conflicting_color {
                    visualizer_updates.push(VisualizerUpdate::SetTitle("3d Medusa: Twice in a Unit".to_string()));
                    for (&(row, col, num), &c) in bicolored_graph.iter() {
                        if c == color {
                            visualizer_updates.push(VisualizerUpdate::ColorCandidate(row, col, num, Colors::CANDIDATE_MARKED_FOR_REMOVAL));
                            reductions.push(SolverAction::CandidateReduction(row, col, num));
            ...
            {
                visualizer_updates.push(VisualizerUpdate::SetTitle("3d Medusa: Two colours in a cell".to_string()));
                for (row, col) in bicolored_graph.keys().map(|&(rowb, colb, _)| (rowb, colb)).unique() {
                    let matching_keys: Vec<_> = bicolored_graph.keys()
                        .filter(|&&(rowb, colb, _)| rowb == row && colb == col)
                        .cloned()
                        .collect();

                    if matching_keys.len() == 0 { continue; }
                    
                    let &first_value = bicolored_graph.get(&matching_keys[0]).unwrap();

                    let all_homogeneous = matching_keys.iter().all(|&key| *(bicolored_graph.get(&key).unwrap()) == first_value);

                    if !all_homogeneous {
                        let candidates_not_in_graph: Vec<usize> = sgrid.candidates[row][col].iter().filter(|&&candidate| !bicolored_graph.contains_key(&(row, col, candidate))).cloned().collect();
                        for candidate in candidates_not_in_graph {
                            ...
                            visualizer_updates.push(VisualizerUpdate::ColorCandidate(row, col, candidate, Colors::CANDIDATE_MARKED_FOR_REMOVAL));
            ...
            {
                visualizer_updates.push(VisualizerUpdate::SetTitle("3d Medusa: Two colours 'elsewhere'".to_string()));
                for num in 1..=9 {
                    ...
                    for (&(row1, col1), &color1) in &temp_map {
                        for (&(row2, col2), &color2) in &temp_map {
                            if (row1, col1) != (row2, col2) && color1 != color2 {
                                let cells_seen_by_both: Vec<(usize, usize)> = SudokuGrid::generate_cells_seen_from_cord((row1, col1))
                                    .intersection(&SudokuGrid::generate_cells_seen_from_cord((row2, col2))).cloned().collect();
                                for (row, col) in cells_seen_by_both {
                                    if (row, col) == (row1, col1) || (row, col) == (row2, col2) { continue; }
            ...
            {
                visualizer_updates.push(VisualizerUpdate::SetTitle("3d Medusa: Two colours Unit + Cell".to_string()));
                for (&(row, col, _), &color) in bicolored_graph.iter() {
                    for &candidate in &sgrid.candidates[row][col] {
                        if bicolored_graph.contains_key(&(row, col, candidate)) { continue; }

                        for (&(rowb, colb, numb), &colorb) in bicolored_graph.iter() {
                            if row == rowb && col == colb { continue; }

                            if color != colorb && numb == candidate && SudokuGrid::cells_see_each_other((row, col), (rowb, colb)) {
                                visualizer_updates.push(VisualizerUpdate::ColorCell(row, col, Colors::CELL_MARKED_FOR_CANDIDATE_REMOVEAL));
                                visualizer_updates.push(VisualizerUpdate::ColorCandidate(row, col, candidate, Colors::CANDIDATE_MARKED_FOR_REMOVAL));
                                reductions.push(SolverAction::CandidateReduction(row, col, candidate));
                            }
            ...
            // Rule 6 Cell Emptied by Color
            {
                    ...
                    for uncolored_cell in iproduct!(0..9, 0..9).filter(|&(row, col)| !bicolored_graph.keys().any(|&(rowb, colb, _)| row == rowb && col == colb)) {
                        ...
                        for cell_seen_by_uncolored_cell in SudokuGrid::generate_cells_seen_from_cord(uncolored_cell) {
                            for &candidate in &sgrid.candidates[uncolored_cell.0][uncolored_cell.1] {
                                if let Some(&color) = bicolored_graph.get(&(cell_seen_by_uncolored_cell.0, cell_seen_by_uncolored_cell.1, candidate)) {
                                    candidate_colors.entry(candidate).or_insert(HashSet::new()).insert(color);
                                } else {
                ...
                if let Some((cell, color)) = conflicting_color {
                    visualizer_updates.push(VisualizerUpdate::SetTitle("3d Medusa: Cell Emptied by Color".to_string()));
                    visualizer_updates.push(VisualizerUpdate::ColorCell(cell.0, cell.1, Colors::CELL_MARKED_FOR_CANDIDATE_REMOVEAL));
                    for (&(row, col, num), &c) in bicolored_graph.iter() {
                        if c == color {
                            visualizer_updates.push(VisualizerUpdate::ColorCandidate(row, col, num, Colors::CANDIDATE_MARKED_FOR_REMOVAL));
                            reductions.push(SolverAction::CandidateReduction(row, col, num));
