...
impl SudokuSolveMethod for IntersectionRemovalSolver {
    fn apply(&self, sgrid: &SudokuGrid) -> Option<SolverResult> {
        for unit_type in [UnitType::Row, UnitType::Col] {
            if let Some(ret) = self.pointing_pair(sgrid, unit_type) {
                return Some(ret);
            };
        }

        for unit_type in [UnitType::Row, UnitType::Col] {
            if let Some(ret) = self.box_line_reduction(sgrid, unit_type) {
                return Some(ret);
            };
        }
        ...
        for unit in &SudokuGrid::get_all_units_from_unit_type(UnitType::Box) {
            for num in 1..=9 {
                ...
                let appeared_in_unit: Vec<(usize, usize)> = unit
                    .iter()
                    .filter(|&&(row, col)| {
                        sgrid.candidates[row][col].contains(&num) && sgrid.grid[row][col] == 0
                    })
                    .cloned()
                    .collect();

                // The cells must all be contained by the pointing direction unit given
                if !SudokuGrid::get_contained_units(&appeared_in_unit)
                    .contains(&pointing_check_unit)
                {
                    continue;
                }
                ...
                visualizer_updates.push(VisualizerUpdate::SetTitle(
                    "Intersection Removal:Pointing Pair".to_string(),
                ));
    ...
    fn box_line_reduction(
        ...
        for unit in &SudokuGrid::get_all_units_from_unit_type(pointing_check_unit) {
            for num in 1..=9 {
                ...
                if !SudokuGrid::get_contained_units(&appeared_in_unit).contains(&UnitType::Box) {
                    continue;
                }
                ...
                visualizer_updates.push(VisualizerUpdate::SetTitle(
                    "Intersection Removal:Box Line".to_string(),
                ));
                ...
                for &(row, col) in
                    SudokuGrid::get_cells_in_unit_from(UnitType::Box, appeared_in_unit[0])
                        .iter()
                        .filter(|&cord| !unit.contains(cord))
                {
                    visualizer_updates.push(VisualizerUpdate::ColorCell(
                        row,
                        col,
