...
impl<const FISH_SIZE: usize> SudokuSolveMethod for SwordfishSolver<FISH_SIZE> {
    fn apply(&self, sgrid: &SudokuGrid) -> Option<SolverResult> {
        for unit_type in [UnitType::Row, UnitType::Col] {
            if let Some(ret) = SwordfishSolver::<FISH_SIZE>::apply_fish_on_axis(sgrid, unit_type) {
                return Some(ret);
            };
        }
        None
...
impl<const FISH_SIZE: usize> SwordfishSolver<FISH_SIZE> {
    pub fn apply_fish_on_axis(sgrid: &SudokuGrid, unit_type: UnitType) -> Option<SolverResult> {
        assert!(
            FISH_SIZE == 3 || FISH_SIZE == 4,
            "Swordfish is fish_size=3 and Jellyfish if fish_size=4. No other types supported."
        );
        for num in 1..=9 {
            ...
            for unit_set in possible_cells_for_fish
                .iter()
                .clone()
                .combinations(FISH_SIZE)
            {
                    ...
                    for &cell_in_fish in cell_set {
                        for (row, col) in SudokuGrid::get_cells_in_unit_from(
                            match unit_type {
                                ...
                                UnitType::Box => {
                                    panic!("Box unit type does not make sense for a fish")
                                }
                            },
                            cell_in_fish,
                        ) {
                            ...
                            match unit_type {
                                UnitType::Row => {
                                    if unit_set.iter().any(|set| set[0].0 == row) {
                                        continue;
                                    }
                                }
                                UnitType::Col => {
                                    if unit_set.iter().any(|set| set[0].1 == col) {
                                        continue;
                                    }
                                }
                                _ => {}
