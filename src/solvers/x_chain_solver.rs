...
use crate::{
    adjacency_graph::AdjacencyGraph, sudoku_grid::SudokuGrid, sudoku_visualizer_builder::Colors,
};
...
impl SudokuSolveMethod for XChainSolver {
    fn apply(&self, sgrid: &SudokuGrid) -> Option<SolverResult> {
        for chain_length in (3..=9).step_by(2) {
            for num in 1..=9 {
                let mut conjugate_pairs: crate::adjacency_graph::AdjacencyGraph =
                    sgrid.get_conjugate_pairs(num);
                let mut is_strong_link: HashMap<
                    ((usize, usize, usize), (usize, usize, usize)),
                    bool,
                > = conjugate_pairs
                    .edges
                    .iter()
                    .flat_map(|(&key, ends)| ends.iter().map(move |&link| ((key, link), true)))
                    .collect();
                    ...
                    for (row, col) in SudokuGrid::generate_cells_seen_from_cord((key.0, key.1)) {
                        if sgrid.candidates[row][col].contains(&num) {
                            if !conjugate_pairs
                                .edges
                                .get(&key)
                                .map_or(false, |neighbors| neighbors.contains(&(row, col, num)))
                            {
                ...
                for start_node in is_strong_link
                    .iter()
                    .filter(|&(_, &value)| value)
                    .flat_map(|(&key, _)| vec![key.0, key.1])
                {
                    let chains = XChainSolver::find_chains_from_node(
                        start_node,
                        &conjugate_pairs,
                        &is_strong_link,
                        chain_length + 1,
                    ...
                    for chain in chains {
                        ...
                        debug_assert!(
                            ...
                            "Chain found and chain length aren't the same."
                        );
                        viualizer_updates
                            .push(SetTitle(format!("X-Chain length {}", chain.len() - 1)));
                        ...
                        for i in 0..chain.len() {
                            ...
                            if i != chain.len() - 1 {
                                let chain_color =
                                    match is_strong_link.get(&(chain[i], chain[i + 1])) {
...
impl XChainSolver {
    ...
    fn dfs(
        ...
        if current_chain.len() == chain_length {
            ...
            if let Some(&val) = is_strong_link.get(&(
                current_chain.last().unwrap().clone(),
                current_chain[current_chain.len() - 2],
            )) {
                if val {
        ...
        if let Some(neighbors) = graph.neighbors(current_node) {
            for &neighbor in neighbors {
                if !visited.contains(&neighbor) {
                    let link = is_strong_link
                        .get(&(current_node, neighbor))
                        .unwrap_or(&false);
