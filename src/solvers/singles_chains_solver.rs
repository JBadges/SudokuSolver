    ...
    fn apply(&self, sgrid: &SudokuGrid) -> Option<SolverResult> {
        for num in 1..=9 {
            ...
            for bicolored_graph in bicolored_graphs.iter() {
                ...
                let mut process_same_color_in_unit =
                    |color_nodes: &HashSet<(usize, usize, usize)>| {
                ...
                for (row, col) in iproduct!(0..9, 0..9).collect::<Vec<(usize, usize)>>() {
                    let sees_red = red_nodes.iter().any(|&red_cell| {
                        SudokuGrid::cells_see_each_other((row, col), (red_cell.0, red_cell.1))
                            && (red_cell.0, red_cell.1) != (row, col)
                    });
                    let sees_blue = blue_nodes.iter().any(|&blue_cell| {
                        SudokuGrid::cells_see_each_other((row, col), (blue_cell.0, blue_cell.1))
                            && (blue_cell.0, blue_cell.1) != (row, col)
                    });
